function [path] = jump_point_search_3D(map, start, goal)
    
    map_start = points_to_idx(map, start);
    map_goal = points_to_idx(map, goal);
    ...
    while isempty(openlist) == 0
        ...
        fa = node.id;
        global testlen;
        testlen = length(openlist);
        
        for j = 1 : size(node.dir)
            ...
            if (map_visit(nodeNew.pos(1), nodeNew.pos(2), nodeNew.pos(3)) == 0)
                ...
                ds = ManhattanDistance(map_goal, nodeNew.pos);
                ManDis = [ManDis ds];
    ...
    while k > 1
        renode = closelist(k);
        float_pos = idx_to_points(map, renode.pos);     
        path = [float_pos; path];
        k = renode.fa;
    end
...
function dis = ManhattanDistance(pos1, pos2)
    pos = pos1 - pos2;
    dis = abs(pos(1)) + abs(pos(2)) + abs(pos(3));
end
...
function ret = outmap(map, pos) 
    ret = false;
    x = pos(1); y = pos(2); z = pos(3);
    if(x < 1 || x > map.nx || y < 1 || y > map.ny || z < 1 || z > map.nz)
        ret = true;
    end
end   
...
function ret = occ_check(map, pos)
    ret = false;
    x = pos(1); y = pos(2); z = pos(3);
    if map.occ_map(x, y, z) == 1
        ret = true;
    end
end
