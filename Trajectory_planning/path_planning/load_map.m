...
n_y = ceil(abs(boundary(5) - boundary(2))/xy_res);%xy_resolution(transform to grid)
n_z = ceil(abs(boundary(6) - boundary(3))/z_res);
map.nx = n_x;
map.ny = n_y;
map.nz = n_z;

map3d = ones(n_x, n_y, n_z, 3)*255;%grid with color
occ_map = logical(zeros(n_x, n_y, n_z));%flag obstacles
for i = 1:size(blocks,1) %the size of first dimension
    block = blocks(i, :);
    ...
    map3d(xs:xe,ys:ye,zs:ze,1) = uint8(block(7));%assign color to block
...
for i = 1:size(blocks,1)
    block = blocks(i, :);
   	xyzs = points_to_idx(map, [block(1) - margin, block(2) - margin, block(3) - margin]);%start point of block belong to which grid
    xs = xyzs(1); ys = xyzs(2); zs = xyzs(3);
...
map.map3d_collision = map3d_collision;% map3d_collision = map3d_collision???
